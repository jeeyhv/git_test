...
QueueNode* MinPriorityQueue :: Meld( QueueNode* a, QueueNode* b )
{
	QueueNode* temp;
	...
	if( a -> Get_cost()  > b -> Get_cost() )// a가 b보다 클 때
	{// a와 b를 swap
		temp = a;
		a = b;
		b = temp;
...
void MinPriorityQueue :: Insert( QueueNode* pNew )
{
	QueueNode *a = root;
	QueueNode * b = pNew;
	QueueNode *temp;
...
void MinPriorityQueue :: clear( QueueNode *p )
{
...
// queue의 root를 pop하고 pop한 root를 return하는 함수 
QueueNode* MinPriorityQueue :: Pop()
{
	QueueNode* temp = root;
	QueueNode* left = root -> Get_left();
	QueueNode* right = root -> Get_right();

	root = NULL;
	
	if( left == NULL )// root의 left child가 없는 경우 
		root = NULL;
	else if( right == NULL )// root의 right child가 없는 경우 
		root = left;
	else // 양쪽 child모두있는 경우 
		Meld( left, right );

	return temp;// root return 
}
